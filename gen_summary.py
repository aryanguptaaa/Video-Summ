...
parser.add_argument('-j', '--json_path', type=str,
                    help='path to json file that stores pred score output by model, it should be saved in score_dir.', default='tvsum_score3/epoch-2.json')
parser.add_argument('-r', '--data_root', type=str,
                    help='path to directory of original dataset.', default='ydata-tvsum50-v1_1')
parser.add_argument('-s', '--save_dir', type=str,
                    help='path to directory where generating results should be saved.', default='Results_tvsum3')
...
def get_keys(id):
    ...
    keyframe_idx = [cps[sel][0] + pred_score[cps[sel][0]: cps[sel][1]].index(max(pred_score[cps[sel][0]: cps[sel][1]])) for sel in pred_selected]
    keyframes = [frames[idx] for idx in keyframe_idx]
